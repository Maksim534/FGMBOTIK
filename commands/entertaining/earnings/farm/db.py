...
async def buy_ferma(user_id: int) -> None:
    ...
    summ = Decimal(balance) - Decimal('30000')
...
async def autoferma() -> None:
    ...
    cursor.execute('UPDATE ferma SET balance = balance + ROUND(1 * POWER(cards, 2.5)) WHERE nalogs < 5000000 AND cards > 0')
    cursor.execute('UPDATE ferma SET nalogs = nalogs + 100000 WHERE nalogs < 5000000')
    conn.commit()
