...
🪓 Для начала тебе нужно будет создать свою ферму, цена постройки 100.000$. Введите команду "Построить ферму" и после через команду "Моя ферма" вы сможете настраивать её и улучшать повышая свою прибыль.
...
async def upd_ferma_text(call: types.CallbackQuery | types.Message, user: BFGuser, action='edit') -> None:
    ...
    dox = int(1 * (ferma.cards.get() ** 2.5)) if ferma.cards.get() != 0 else 3000
    ch = int(200000 * (1 + 0.15) ** ferma.cards.get())
...
async def buy_cards_cmd(call: types.CallbackQuery, user: BFGuser):
    ...
    ch = int(200_000 * (1 + 0.15) ** (ferma.cards.get() - 1))
