...
🪓 Для начала тебе нужно будет создать свою ферму, цена постройки 200000$. Введите команду "Построить ферму" и после через команду "Моя ферма" вы сможете настраивать её и улучшать повышая свою прибыль.
...
async def upd_ferma_text(call: types.CallbackQuery | types.Message, user: BFGuser, action='edit') -> None:
    ...
    dox = int(2 * (ferma.cards.get() ** 1.5)) if ferma.cards.get() != 0 else 2
    ch = int(220000 * (1 + 0.77) ** ferma.cards.get())
...
async def buy_cards_cmd(call: types.CallbackQuery, user: BFGuser):
    ...
    ch = int(205_000 * (1 + 0.80) ** (ferma.cards.get() - 1))
