...
🪓 Для начала тебе нужно будет построить свой сад, цена постройки 5.000.000.000$. Введите команду "Построить сад" и после через команду "Мой сад" вы сможете настраивать его и улучшать повышая свою прибыль.
...
async def sell_garden(message: types.Message, user: BFGuser):
    ...
    summ = 2_500_000  # Половина стоимости сада
    
    for i in range(1, garden.tree.get() + 1):  # Компенсация за деревья (50%)
        summ += int(5_000_000_000 * (1 + 0.15) ** (i + 1)) // 2
