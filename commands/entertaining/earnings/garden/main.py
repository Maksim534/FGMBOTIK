...
🪓 Для начала тебе нужно будет построить свой сад, цена постройки 40.000.000.000$. Введите команду "Построить сад" и после через команду "Мой сад" вы сможете настраивать его и улучшать повышая свою прибыль.
...
async def upd_garden_text(call: types.CallbackQuery | types.Message, user: BFGuser, action='edit') -> None:
    ...
    ch = int(40_000_000_000 * (1 + 0.15) ** (garden.tree.get() + 1))
...
async def sell_garden(message: types.Message, user: BFGuser):
    ...
    summ = 20_000_000_000  # Половина стоимости сада
    
    for i in range(1, garden.tree.get() + 1):  # Компенсация за деревья (50%)
        summ += int(40_000_000_000 * (1 + 0.15) ** (i + 1)) // 2
